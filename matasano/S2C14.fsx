...
//let randomPrefix = randomBytes r (int((randomBytes r 1).[0]))
let randomPrefix = Array.zeroCreate<byte> 0

let encrypt (bytes: byte[]) : (byte []) =
    let unknownBytes = base64Decode unknownBase64Text
    let bytesToEncrypt = [randomPrefix;bytes;unknownBytes] |> Array.concat 
    let blockSize = 16
...
let detectBlockSize encrypt =
    ...
    // byte arrays of varying lengths to determine block boundary
    let userBytes = blockSizes |> Array.map (fun blockSize -> Array.zeroCreate<byte> blockSize)
    ...
    // find first time where encrypted size increases by an entire block
    let (blockSize, _) = ts |> Seq.find (fun (cul0, cel0) -> (cel0 - restEncryptedLength) > restBlockSize)

    let detectedBlockSize = blockSize - restBlockSize
    detectedBlockSize
...
let getMap bytes offset blockSize =

    let getMapPart (bs: byte []) =
        let encrypted = encrypt bs
        let encryptedBlock = Array.sub encrypted offset blockSize
        let lastByte = bs.[bs.Length - 1]
        (encryptedBlock, lastByte)

    let theMap =
        bytes
        |> Array.map getMapPart
        |> Map.ofArray
    theMap
...
let getSecretBytes blockSize blockCount =
    ...
    ts |> Array.fold (fun (secretBytes:byte[]) ((blockNumber, paddingSize:int):(int*int)) ->
        ...
        let blockPrefix = Array.zeroCreate<byte> prefixLength

        let blockSuffixes = [|0uy..255uy|]
        let plaintextBytes =
            blockSuffixes
            |> Array.map (fun b -> [blockPrefix;secretBytes;[|b|]] |> Array.concat)
...
let doit () =
    ...
    printfn "Block size: %d" detectedBlockSize
